package org.example.blogajax.rest_controller;

import org.example.blogajax.dto.BlogDto;
import org.example.blogajax.model.Blog;
import org.example.blogajax.service.IBlogService;
...
//    }
    ...
    public ResponseEntity<List<Blog>> search(@RequestParam String keyword) {
        List<Blog> blogList = blogService.searchBlogsByTitle(keyword);
        if (blogList.isEmpty()) {
            return new ResponseEntity<>(HttpStatus.NO_CONTENT);
        }
        return new ResponseEntity<>(blogList,HttpStatus.OK);
